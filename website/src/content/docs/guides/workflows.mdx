---
title: "Workflows"
description: "Practical Trello workflows expressed through the MCP tool catalog."
---
...
:::caution[Client approval is not a server guarantee]
`trello-mcp` executes a valid
...
[Set up your MCP client](/getting-started/clients/) first if the tools are not visible, or
[Trello API key](/getting-started/trello-api-key/) if the credentials are not configured.
...
:::caution[Keep the safety boundary visible]
Treat each workflow as a sequence of
...
- Review the [API Coverage](/reference/api-coverage/) page when a prompt assumes a
