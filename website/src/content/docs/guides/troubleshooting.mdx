...
description: "Diagnose startup, transport, authentication, Docker, Trello API, rate-limit, and attachment failures safely."
editUrl: "https://github.com/enthouan/trello-mcp/edit/main/docs/troubleshooting.md"
---
...
:::danger[Sanitize before sharing]
Keep real credentials, authorization headers,
...
[environment table](/reference/configuration/).
...
3. **Restart after configuration changes.** The server reads its environment at
   ...
   client as described in [Set up your MCP client](/clients/).
...
- Check the client-specific configuration shape in
  [Set up your MCP client](/clients/); field names are not interchangeable
  between clients.
...
[Set up your MCP client](/clients/#mcp-inspector-and-manual-clients).
...
in [MCP Client Compatibility](/clients/compatibility/). Some clients group
...
mount example in [Attachment Uploads](/reference/configuration/#local-attachment-uploads), or add
...
- If the token was revoked, expired, exposed, or belongs to the wrong member,
  replace it using [Trello API Key](/trello-api-key/).
...
[SECURITY.md](/security/). For ordinary contribution guidance, see
[CONTRIBUTING.md](https://github.com/enthouan/trello-mcp/blob/main/CONTRIBUTING.md).
