...
3. **Restart after configuration changes.** The server reads its environment at
   ...
   client as described in [Set up your MCP client](/getting-started/clients/).
...
- Check the client-specific configuration shape in
  [Set up your MCP client](/getting-started/clients/); field names are not interchangeable
  between clients.
...
[Set up your MCP client](/getting-started/clients/#mcp-inspector-and-manual-clients).
...
in [MCP Client Compatibility](/getting-started/compatibility/). Some clients group
...
- If the token was revoked, expired, exposed, or belongs to the wrong member,
  replace it using [Trello API key](/getting-started/trello-api-key/).
